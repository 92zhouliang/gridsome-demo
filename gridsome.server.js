...
module.exports = function (api) {
  ...
  api.loadSource(async ({ addCollection }) => {
    ...
    const result = await axios({
      url: baseUrl + "/users/GitHub-Laziji/gists?page=1&per_page=1",
      type: 'get'
    })
    // console.log(result)
    ...
    const result_1 = await axios({
      url: baseUrl + "/users/GitHub-Laziji",
      type: 'get'
    })
    ...
    const result3 = await axios({
      url: baseUrl + "/users/GitHub-Laziji/followers?page=1&per_page=9999999999999",
      type: 'get'
    })
    ...
    const result3_2 = await axios({
      url: baseUrl + "/users/GitHub-Laziji/following?page=1&per_page=9999999999999",
      type: 'get'
    })
    ...
    //博客列表
    {
      ...
      const detail = await axios({
        url: baseUrl + "/users/GitHub-Laziji/gists?page=1&per_page=9999999999999",
        type: 'get'
      })
      const file = detail.data.files
      console.log(detail)
    ...
    //开源项目
    {
      ...
      const detail = await axios({
        url: baseUrl + "/users/GitHub-Laziji/repos?page=1&per_page=9999999999999",
        type: 'get'
      })
      for (let item of detail.data) {
        collection5.addNode({
